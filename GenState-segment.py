...
class AllModel(nn.Module):
    ...
    def forward(self, data, data_mask, e_adj_matrix,transition,label_inx_positive,label_inx,mask):
        ...
        train_all, loss_a, datax = self.GCN(GNN_data,e_adj_matrix,label_inx,mask)
        return train_all, loss_a, datax
...
run_model = torch.load('Pre_TrGcn_model1.pth').cuda()
...
run_model.eval()
with torch.no_grad():
    for name in range(0,len(seg)):

        if os.name == 'nt':
            #packets = rdpcap(os.getcwd()+"\\valid_set\\"+fileh_valid[name])
            #analysis_set
            packets = rdpcap(os.getcwd() + "\\analysis_set\\DoS-DDoS-MQTT-IoT\\WILL_DDoS_AD_1.pcap")
        else:
            #packets = rdpcap(os.getcwd()+"/valid_set/"+fileh_valid[name])
            packets = rdpcap(os.getcwd() + "/analysis_set/DoS-DDoS-MQTT-IoT/" + "WILL_DDoS_AD_1.pcap")
        ...
        if 1:
            ...
            for packet in packets[0:seg[name]]:  # [0:100]:
                #parse MQTT
            ...
            print("This is valid set:","fully edge num:", len(edge_full),"real edge num:", len(edge))
            ...
            print(f"e_adj edges num:{adj_fully_len} adj edges num:{adj_len}")
            print("len(predicted_labels)",len(predicted_labels))

            c_e = 0
            c_s = 0
            state = []
            for c in range(0,adj_fully.shape[0]):
                for r in range(0, adj_fully.shape[1]):
                    if adj_fully[c][r] == 1:
                        adj_fully[c][r] = predicted_labels[c_e]
                        if predicted_labels[c_e] == 1:
                            #print(mqtte[c],"--[",mqtte[r][0][0],"]-->",mqtte[r],"\n")
                            state.append([mqtte[c],"--(",mqtte[r][0][0],")-->",mqtte[r]])
                            with open('output_pre_'+str(seg[name])+'.txt', 'a', encoding='utf-8') as file:
                                file.write(str(mqtte[c][0])+"--("+str(mqtte[r][0][0])+")-->"+str(mqtte[r][0])+"\n")
                            c_s += 1
                        c_e += 1
            c_s_r = 0
            for c in range(0,adj.shape[0]):
                for r in range(0, adj.shape[1]):
                    if adj[c][r] == 1:
                        with open('output_real_'+str(seg[name])+'.txt', 'a', encoding='utf-8') as file:
                            file.write(str(mqtte[c][0])+"--("+str(mqtte[r][0][0])+")-->"+str(mqtte[r][0])+"\n")
                            c_s_r += 1
            print("c_s_r:", c_s_r)
            print("c_s:", c_s)
