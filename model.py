...
class BertConfig(object):
    ...
    def __init__(self,
                 vocab_size, # 字典字数
                 hidden_size, # 隐藏层维度也就是字向量维度
                 num_hidden_layers, # transformer block 的个数
                 num_attention_heads, # 注意力机制"头"的个数
                 intermediate_size=10*4, # feedforward层线性映射的维度
                 hidden_act="relu", # 激活函数
                 hidden_dropout_prob=0.2, # dropout的概率
...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.word_embeddings = nn.Embedding(config.vocab_size, config.hidden_size,padding_idx=0)#padding_idx=0
    ...
    def forward(self, input_ids, token_type_ids=None):#, positional_enc
        """
        ...
        #print('input_ids = ',input_ids.type(),input_ids.shape)
        #print(input_ids)
        # 字向量查表
        words_embeddings = self.word_embeddings(input_ids)
        #print('embedding table:',words_embeddings)
        if token_type_ids is None:
            token_type_ids = torch.zeros_like(input_ids)
        token_type_embeddings = self.token_type_embeddings(token_type_ids)

        embeddings = words_embeddings + token_type_embeddings#+ positional_enc
...
class BertSelfAttention(nn.Module):
    ...
    def transpose_for_scores(self, x):
        ...
        #print('transpose_for_scores:', x.size()[:-1])
    ...
    def forward(self, hidden_states, attention_mask, get_attention_matrices=False):
        ...
        #print(f"query_layer={query_layer.type(), query_layer.shape} key_layer={key_layer.transpose(-1, -2).type(), key_layer.transpose(-1, -2).shape}")
        ...
        attention_probs= attention_probs_
        # 用注意力矩阵加权V
...
#, positional_enc
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
        ...
        #print('go into embedding',input_ids)
        # embedding层, positional_enc , token_type_ids
